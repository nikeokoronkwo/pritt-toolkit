import {
  PrittAdapterArchiveRequestFunc,
  PrittAdapterMetaRequestFunc,
  PrittAdapterResolveFunc,
} from "./types/adapter";
import {
  Config,
  PackageManager,
  PrittContext,
  PrittLocalConfigUnawareController,
...
export interface PrittHandler {
  ...
  onGetConfig: (
    directory: string,
    controller: PrittLocalConfigUnawareController,
  ) => Promise<Config | undefined> | (Config | undefined);

  onGetWorkspace: (
    directory: string,
    controller: PrittLocalController,
  ) => Promise<Workspace | undefined> | (Workspace | undefined);

  onConfigure: (
    context: PrittContext,
    controller: PrittLocalController,
  ) => Promise<any> | any;
}
